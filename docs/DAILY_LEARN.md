...
**Skill.** A JSONL object line with single-quoted strings is ERROR (invalid JSONL). RFC JSON requires double quotes; `json.loads` rejects `{'timestamp': '...'}`. Spaces after `}` still parse; quotes are not rewritten.

**Why.** Without a named single-quote lock, a later JSON5/Python-literal parse would silently accept `'...'` events. This is not a boolean-field enum.
...
**Retrieve.** `src/constraintauditor/journal.py` · `tests/test_journal.py` · `docs/ADAPTER.md` · `examples/MATRIX.md` · `LOOP_STATE.md` NEXT TICK W365
