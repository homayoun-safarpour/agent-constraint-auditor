...
def test_adapter_locks_whitespace_only_jsonl_parses_to_no_events():
    assert "A whitespace-only JSONL file is the same" in ADAPTER
...
def test_adapter_locks_non_string_jsonl_text_is_treated_as_missing():
...
def test_adapter_locks_whitespace_jsonl_text_with_fields_uses_fields():
    assert "Whitespace-only JSONL `text` with `fields` uses the fields body" in ADAPTER


def test_adapter_locks_empty_string_jsonl_text_with_fields_uses_fields():
...
def test_adapter_locks_boolean_fields_values_are_stringified():
    assert "A JSONL `fields` mapping with boolean values still parses" in ADAPTER
...
def test_adapter_locks_empty_string_fields_keys_still_parse():
    assert "A JSONL `fields` mapping with an empty-string key still parses" in ADAPTER
...
def test_adapter_locks_whitespace_only_fields_values_still_parse():
    assert "A JSONL `fields` mapping with a whitespace-only value still parses" in ADAPTER
    assert "the value is not stripped" in ADAPTER


def test_adapter_locks_empty_string_fields_values_still_parse():
    assert "A JSONL `fields` mapping with an empty-string value still parses" in ADAPTER
    assert "that value is not missing fields" in ADAPTER


def test_adapter_locks_fields_keys_containing_a_colon_still_parse():
    assert "A JSONL `fields` mapping with a key containing a colon still parses" in ADAPTER
    assert "the key is not split" in ADAPTER


def test_adapter_locks_fields_values_containing_a_colon_still_parse():
    assert "A JSONL `fields` mapping with a value containing a colon still parses" in ADAPTER
...
def test_adapter_locks_jsonl_object_line_trailing_whitespace_still_parses():
    assert "A JSONL object line with trailing whitespace still parses" in ADAPTER
    assert "spaces after the closing brace are stripped" in ADAPTER


def test_adapter_locks_jsonl_object_line_leading_whitespace_still_parses():
    assert "A JSONL object line with leading whitespace still parses" in ADAPTER
    assert "spaces before the opening brace are stripped" in ADAPTER
...
def test_adapter_locks_jsonl_trailing_comma_before_brace_is_invalid_jsonl():
    assert "A JSONL object line with a trailing comma before the closing brace is ERROR (invalid JSONL)" in ADAPTER
...
def test_adapter_locks_jsonl_unquoted_string_values_are_invalid_jsonl():
    assert "A JSONL object line with an unquoted string value is ERROR (invalid JSONL)" in ADAPTER
    assert "unquoted values are not accepted" in ADAPTER
